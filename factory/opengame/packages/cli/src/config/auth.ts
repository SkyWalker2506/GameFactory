...
export function validateAuthMethod(authMethod: string): string | null {
  ...
  if (
    authMethod === AuthType.USE_CLAUDE_CLI ||
    authMethod === AuthType.USE_CODEX_CLI ||
    authMethod === AuthType.USE_GEMINI_CLI ||
    authMethod === AuthType.USE_OPENCODE_CLI
  ) {
    // Subscription CLI adapters don't need API keys — auth lives in the CLI itself.
    return null;
  }
