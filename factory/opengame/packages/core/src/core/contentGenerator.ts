...
export enum AuthType {
  ...
  USE_CODEX_CLI = 'codex-cli',
  USE_GEMINI_CLI = 'gemini-cli',
  USE_OPENCODE_CLI = 'opencode-cli',
}
...
export function createContentGeneratorConfig(
  ...
  if (
    authType === AuthType.USE_CLAUDE_CLI ||
    authType === AuthType.USE_CODEX_CLI ||
    authType === AuthType.USE_GEMINI_CLI ||
    authType === AuthType.USE_OPENCODE_CLI
  ) {
    // Subscription CLI adapters require no API key — auth lives inside the CLI.
    // Model defaults: claude → opus, codex → gpt-5.4, gemini → pro.
    // OpenCode defers to its own configured default model unless overridden.
    const defaultModel =
      authType === AuthType.USE_CLAUDE_CLI
        ? process.env['CLAUDE_CLI_MODEL'] ?? 'claude-opus-4-7'
        : authType === AuthType.USE_CODEX_CLI
        ? process.env['CODEX_CLI_MODEL'] ?? 'gpt-5.4'
        : authType === AuthType.USE_GEMINI_CLI
        ? process.env['GEMINI_CLI_MODEL'] ?? 'gemini-3-pro-preview'
        : process.env['OPENCODE_CLI_MODEL'];
    newContentGeneratorConfig = {
      ...newContentGeneratorConfig,
      model: newContentGeneratorConfig.model || defaultModel,
      apiKey: 'cli-no-key',
    };
  }
...
export async function createContentGenerator(
  ...
  if (
    config.authType === AuthType.USE_CLAUDE_CLI ||
    config.authType === AuthType.USE_CODEX_CLI ||
    config.authType === AuthType.USE_GEMINI_CLI ||
    config.authType === AuthType.USE_OPENCODE_CLI
  ) {
    const { createCliContentGenerator } = await import(
      './cliContentGenerator/index.js'
    );
    const generator = createCliContentGenerator(config, gcConfig);
    return new LoggingContentGenerator(generator, gcConfig);
  }
